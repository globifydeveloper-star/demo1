  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center gap-3 py-6">
        ...
        <p className={`text-sm ${isDark ? "text-white/60" : "text-muted"}`}>Nos experts Shopify vous contacteront dans les 24 heures.</p>
      </div>
    );
  }
    ...
    <div className="w-full max-w-md">
      {step === 1 ? (
        <form onSubmit={handleStep1} className="flex flex-col sm:flex-row gap-3">
          <Input type="email" required placeholder="Votre adresse e-mail" value={email} onChange={(e) => setEmail(e.target.value)} className={`flex-1 h-12 rounded-full px-5 ${inputCls}`} />
        ...
        <form onSubmit={handleStep2} className="flex flex-col gap-3">
          <p className={`text-xs font-medium ${isDark ? "text-white/60" : "text-muted"}`}>Presque fini — dites-nous en un peu plus :</p>
          <Input required name="name" placeholder="Votre nom *" className={`h-11 rounded-lg px-4 ${inputCls}`} />
          <Input required name="phone" type="tel" placeholder="Numéro de téléphone *" className={`h-11 rounded-lg px-4 ${inputCls}`} />
          <Input name="company" placeholder="Nom de l'entreprise" className={`h-11 rounded-lg px-4 ${inputCls}`} />
          <select required name="revenue" className={`h-11 rounded-lg px-4 text-sm border ${isDark ? "bg-white/10 border-white/20 text-white" : "bg-foreground/5 border-border text-foreground"}`}>
            <option value="">Chiffre d'affaires mensuel</option>
            <option>Moins de 25 000 €</option>
          ...
          <Button type="submit" disabled={isSubmitting} className="h-11 rounded-full bg-primary text-primary-foreground hover:bg-primary/90 font-semibold gap-2">
            ...
            {isSubmitting ? "Envoi..." : "Obtenir mon plan de croissance"}
          </Button>
        </form>
      )}
    </div>
    ...
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial="hidden" animate="visible" variants={stagger} className="space-y-6">
              <motion.div variants={fadeUp} className="inline-flex items-center gap-2 bg-white border border-border/20 rounded-full px-4 py-1.5 text-xs font-medium text-foreground shadow-sm">
                Experts Shopify en France — Globify
              </motion.div>
              <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl lg:text-[3.4rem] font-extrabold leading-[1.1] text-white">
                Lancez votre <span className="bg-gradient-to-r from-primary to-orange-400 bg-clip-text text-transparent">Boutique Shopify</span> en France
              </motion.h1>
              <motion.p variants={fadeUp} className="text-lg text-white/70 max-w-xl leading-relaxed">
                Propulsez votre marque avec l'agence leader en développement Shopify en France. Du design sur mesure à l'intégration des paiements locaux — nous créons des boutiques qui vendent.
              </motion.p>
              <motion.div variants={fadeUp} className="flex flex-wrap gap-4 text-sm text-white/60">
                {["Partenaires Officiels", "Paiements Locaux", "Intégration Logistique", "Optimisé pour la Conversion"].map((t) => (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4, duration: 0.7 }} className="hidden lg:block relative">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 relative overflow-hidden group hover:bg-white/10 transition-colors">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h3 className="text-white font-bold">Expertise Locale</h3>
                    <p className="text-white/40 text-sm">Paris, Lyon, Marseille</p>
                  </div>
                </div>
                <div className="space-y-4">
                  {[
                    { icon: CreditCard, title: "Paiements France", desc: "Carte Bancaire, Stripe, PayPal, Klarna." },
                    { icon: Truck, title: "Logistique France", desc: "Colissimo, Chronopost, Mondial Relay." },
                    { icon: Languages, title: "Localisation", desc: "Design & SEO optimisés pour la France." },
                  ].map((item, i) => (
      ...
      <section className="bg-card border-b border-border py-6">
        <div className="container mx-auto px-6">
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
            {[
              { val: "85+", label: "Boutiques Lancées" },
              { val: "20M €+", label: "Ventes Traitées" },
              { val: "Support Local", label: "Équipe en France" },
              { val: "4.9/5", label: "Note Clients" },
              { val: "Shopify", label: "Agence Partner" },
            ].map((s) => (
      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="container mx-auto px-6 text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-8 text-foreground">Prêt à propulser votre marque en France ?</h2>
          <p className="text-muted-foreground mb-12 max-w-2xl mx-auto">Rejoignez les marques leaders en France qui font confiance à Globify pour leur succès sur Shopify. Obtenez votre session stratégique gratuite aujourd'hui.</p>
      ...
      <section className="py-20 md:py-28 section-dark relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="max-w-2xl mx-auto">
            <motion.h2 variants={fadeUp} className="text-3xl md:text-4xl font-bold text-white mb-6">Créez une boutique que vos clients vont adorer</motion.h2>
            <motion.p variants={fadeUp} className="text-white/60 mb-10">Arrêtez de deviner et commencez à vendre. Notre équipe Shopify est prête à propulser votre marque.</motion.p>
            <motion.div variants={fadeUp} className="flex justify-center mb-8"><InlineLeadForm id="final-fr" variant="dark" /></motion.div>
            <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-4">
              <Button onClick={openContactDialog} variant="outline" className="rounded-full px-8 h-12 border-primary text-primary hover:bg-primary hover:text-white gap-2 font-semibold">Réserver un appel</Button>
              <a href="https://wa.me/971547308673?text=Bonjour%20Globify%2C%20je%20suis%20intéressé%20par%20le%20développement%20Shopify%20en%20France." target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full px-8 h-12 bg-[#25D366] text-white font-semibold hover:bg-[#22c55e] transition-colors" onClick={() => typeof window !== "undefined" && (window as any).gtag && (window as any).gtag('event', 'contact_whatsapp')}><MessageCircle className="w-5 h-5" /> WhatsApp</a>
            </motion.div>
          </motion.div>
        </div>
      </section>
